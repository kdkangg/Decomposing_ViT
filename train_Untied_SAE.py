...
parser.add_argument('--vit', type=str, help='ViT model name', default = 'nateraw/vit-base-patch16-224-cifar10')
parser.add_argument('--modelpath', type=str, help='Path to save model', default ="cifar10")
parser.add_argument('--actpath', type=str, help='Path to saved activations', default = "activations_cifar10_vit_b")
...
optimizer = torch.optim.Adam([untied_sae.encoder, untied_sae.encoder_bias, untied_sae.decoder], lr=args.lr)
original_bias = untied_sae.encoder_bias.clone().detach()
...
writer = SummaryWriter(f'runs/{args.modelpath}/SAE_ratio{args.ratio}_epochs{args.epochs}_lr{args.lr}_untied')
dead_features = torch.zeros(untied_sae.encoder.shape[0])
...
for epoch in tqdm(range(args.epochs)):
    for i, (images, activations) in enumerate(train_loader):
        untied_sae.encoder.requires_grad = True
        untied_sae.encoder_bias.requires_grad = True
        untied_sae.decoder.requires_grad = True

        with torch.no_grad():
            activations = activations.to(device)
        
        c = untied_sae.encode(activations)
        x_hat = untied_sae.decode(c)
    ...
    # validation
    untied_sae.encoder.requires_grad = False
    untied_sae.encoder_bias.requires_grad = False
    untied_sae.decoder.requires_grad = False
    ...
    if total_loss < best_loss:
        ...
        counter = 0
        # Save the best model
        torch.save(untied_sae.state_dict, f'SAE_models/{args.modelpath}/SAE_ratio{args.ratio}_epoch{args.epochs}_lr{args.lr}_untied.pth')
    else:
        counter += 1
